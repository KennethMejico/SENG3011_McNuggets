...
def test_findDate(setup):
    assert(setup["scraper"].findDate(setup["res1"]) == "28 Feb 2021")
    assert(setup["scraper"].findDate(setup["res2"]) == "01 Sep 2020")

def test_findID(setup):
    assert(setup["scraper"].findID(setup["res1"]) == "8219231")
    assert(setup["scraper"].findID(setup["res2"]) == "7823938")

def test_findName(setup):
    assert(setup["scraper"].findName(setup["res1"]) == "Ebola update (20): USA, Guinea, Sub-Saharan Africa")
    assert(setup["scraper"].findName(setup["res2"]) == "Announcement (04): ProMED anniversary award 2020")

def test_printInfo(capsys):
    setup["scraper"].printInfo(100, "28 Feb 2021", "8219231", "Ebola update (20): USA, Guinea, Sub-Saharan Africa")
...
def test_processData(capsys):
    ...
    setup["scraper"].processData(raw_data)
    out, err = capsys.readouterr()
